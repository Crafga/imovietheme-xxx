			...
			{:else}
				...
				<div class="flex justify-center gap-2 p-4">
					...
					{#each pageList as page}
						...
						{:else}
							<button on:click={() => goto(`/categories/${data.cateName}?page=${page}`)} class="btn btn-sm {page === currentPage ? 'variant-filled-primary' : 'variant-ghost-surface'}" disabled={currentPage === page}>
								{page}
							</button>
						  {/if}
					{/each}
