<script lang="ts">
    ...
    $: randomTags = getRandomItems(data.tags?.tags as Tag[], 20);
	...
	// SEO Configuration for specific tag page
	$: seoConfig = {
		title: createSeoTitle(`หนัง ${data.tagName}`, data.settings?.title ?? 'iMovie'),
		description: truncateDescription(`ดูหนัง ${data.tagName} ออนไลน์ฟรี คุณภาพ HD ซับไทย พากย์ไทย รวมหนังแท็ก ${data.tagName} ยอดฮิต`),
		keywords: [
			data.tagName,
			`หนัง ${data.tagName}`,
			'ดูหนังออนไลน์',
...
<svelte:head>
	<!-- Tag Specific Page SEO Meta Tags -->
		...
		<div class="flex-1 mt-10">
			...
			<div class="card p-4 mb-6 rounded-lg">
                ...
                <div class="flex flex-col md:flex-row justify-center items-center md:items-center gap-4">
					<div>
						<h1 class="text-2xl font-bold mb-2">
							{#each randomTags as t}
								<a href="/tags/{t.name}">
									<span class="badge variant-soft-secondary hover:variant-soft-success mr-2">{t.name}</span>
								</a>
							{/each}
						</h1>
					</div>
